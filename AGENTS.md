...
`.cursor/environment.json` provisions a self-contained local dev stack for Cloud
Agents. `.cursor/install.sh` installs PostgreSQL, ffmpeg, and the MinIO
server/client, then runs `pnpm install`. `.cursor/start.sh` boots PostgreSQL and
MinIO, writes a git-ignored `.env`, and applies migrations. The `web` terminal
runs `pnpm dev:web` on port 3000.

- PostgreSQL is local and Railway-compatible:
 `postgres://lumenclip:lumenclip@127.0.0.1:5432/lumenclip`.
- MinIO stands in for the private Railway bucket (`lumenclip-assets`) at
 `http://127.0.0.1:9000` using path-style addressing.
- Provider keys (for example `OPENROUTER_API_KEY`) and Clerk keys are not baked
 in; add them as Cloud Agent Secrets. Clerk runs in keyless dev mode without a
 publishable key, so the app boots and auth boundaries still apply.
- `.env` is regenerated by `start.sh`; load it before running scripts or tests
 that read `process.env` directly, e.g. `set -a; . ./.env; set +a`.
- Both scripts are idempotent. If services are not up, re-run
