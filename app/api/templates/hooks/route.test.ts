...
// Appwrite-only: `data/templates/automations.json` -> `automations`, run
// against cfarm (forced by vitest.setup.ts), cleared between tests.
let tempRoot: string

const clearAutomations = () => clearTestTables("templates")
...
describe("POST /api/templates/hooks", () => {
  it("generates ten hooks from a sampled user automation and persists them", async () => {
    ...
    await upsertAutomationRecords({
      rootDir: path.join(tempRoot, "data", "templates"),
      records: [automation],
    })
    ...
    const response = await POST(
      new Request("http://localhost/api/templates/hooks", {
        method: "POST",
        body: JSON.stringify({ templateId: automation.id }),
      })
    )
