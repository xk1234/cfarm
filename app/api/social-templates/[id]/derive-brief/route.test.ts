...
const clearAutomations = () => clearTestTables("social_templates")
...
describe("POST /api/social-templates/[id]/derive-brief", () => {
  it("persists bounded provider diagnostics and returns a retryable error", async () => {
    ...
    const response = await POST(
      new Request("http://localhost/api/social-templates/test/derive-brief", {
        method: "POST",
      }),
