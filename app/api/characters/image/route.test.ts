...
beforeEach(async () => {
  tempRoot = await mkdtemp(path.join(os.tmpdir(), "cfarm-character-image-route-"))
  await mkdir(path.join(tempRoot, "data", "characters", "headshots"), { recursive: true })
  await writeFile(path.join(tempRoot, "data", "characters", "headshots", "maya.png"), new Uint8Array([137, 80, 78, 71]))
...
describe("POST /api/characters/image", () => {
  it("generates a character image with the selected aspect ratio and profile attachment", async () => {
    const fetchMock = vi.fn()
      ...
      .mockResolvedValueOnce(new Response(JSON.stringify({ data: { taskId: "character-image-task-1" } }), { status: 200 }))
      .mockResolvedValueOnce(new Response(JSON.stringify({
        data: {
          successFlag: 1,
          response: { resultImageUrl: "https://example.com/generated-character-image.png" },
        },
      }), { status: 200 }))
    ...
    const response = await POST(new Request("http://localhost/api/characters/image", {
      method: "POST",
      body: JSON.stringify({
    ...
    expect(createRequest.inputImage).toBe("https://kie.example.com/maya-reference.png")
    expect(createRequest.aspectRatio).toBe("4:5")
    expect(createRequest.prompt).toContain("film this in a kitchen")
    expect(payload.imageUrl).toMatch(/\/api\/local-assets\/characters\/images\/\d+-character-image-task-1\.png/)
    ...
    const stored = JSON.parse(await readFile(path.join(tempRoot, "data", "characters", "images.json"), "utf8"))
