...
description: Set an template's posting times, weekdays, timezone, and weekly frequency.
---
...
The Schedule section is the saved cadence for one template. It shows the
template timezone, a frequency summary, total posts per week, and up to five
...
restores the saved template schema and returns to Overview; Save Changes
persists the cadence and returns to Overview. Pausing the template prevents
...
`lumenclip_template_update` changes timezone, posting times, jitter, and
