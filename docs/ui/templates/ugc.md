...
unless `ENABLE_UGC_AUTOMATION` is exactly `true`. The template must also be an
explicitly enabled UGC template. Its editor states that generation requires
...
The template editor links each persisted UGC run to this route. Retry from
