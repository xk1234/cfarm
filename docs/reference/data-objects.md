...
**Persistence model.** Mutable stores read/write Appwrite Cloud TablesDB via `lib/json-store.ts` (`readJsonArrayStore` / `writeJsonArrayStore` / `withJsonArrayStore` / `upsertJsonArrayRecord` / `appendJsonArrayRecords` / `deleteJsonArrayRecord`), with row order preserved by the `ord` column. **There is no filesystem fallback for mapped stores:** Appwrite is authoritative, and an unmapped store or unconfigured Appwrite is a hard error (`requireTableFor`). See `lib/appwrite-stores.ts` for the current store→table map. `RealFarmData` (`data/realfarm.json`) is a static config seed compiled in via `import`, not a mutable store, and stays on the filesystem by design. Binary asset *files* are a separate matter — they mirror to Storage buckets but keep local copies as working files for ffmpeg/sharp (see AssetRecord).

> This doc covers the core objects. Newer objects live in their subsystem libs and are not all cataloged here yet: `PostFastMetricSnapshot` / `AccountFollowerSnapshot` (`lib/postfast-metric-snapshots.ts`), `CalendarItem` (`lib/calendar-items.ts`), `KnowledgeBaseRecord` (`lib/knowledge-bases.ts`), X-automation records (`x_automations`, `x_automation_runs`), `ProductCollection`, `media_library`, and the benchmark corpora/scores.
...
Persistence: Appwrite `assets` table (via `lib/json-store.ts`) — authoritative, no filesystem fallback. Binary files mirror to the `assets` Storage bucket (see below).
...
Persistence: Appwrite `generated_video_exports` table (via `lib/json-store.ts`) — authoritative, no filesystem fallback.
...
Persistence: Appwrite `image_collections` table (via `lib/json-store.ts`) — authoritative, no filesystem fallback.
...
Persistence: Appwrite `automations` table (via `lib/json-store.ts`) — authoritative, no filesystem fallback.
...
Persistence: Appwrite `postfast_posts` table (via `lib/json-store.ts`) — authoritative, no filesystem fallback.
...
Persistence: Appwrite `characters` table (via `lib/json-store.ts`) — authoritative, no filesystem fallback. Character image/video generations persist too, in the `character_generations` and `character_video_generations` tables.
...
Persistence: Appwrite `swipes` table (via `lib/json-store.ts`) — authoritative, no filesystem fallback. Screenshots/media mirror to the `misc` Storage bucket and serve via `/api/swipes/assets/[file]`.
