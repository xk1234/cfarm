...
| `POST /api/assets/generate`                  | JSON `{ kind?, scope?, category?, name?, prompt, model? }`                      | Creates an image placeholder or records a failed request; `201` | Compatibility stub; only image placeholders are currently produced |
