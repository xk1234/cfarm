...
Every trigger below generates an unpublished draft against a saved slideshow automation. None of
...
`automationId` is required; `requestId` and `hook` are optional. A supplied `hook` replaces random
selection from the saved pool without changing the pool. The tool rejects any automation whose kind
is not `slideshow`.

`lumenclip_automation_run` runs the same pipeline and additionally accepts X, Threads, and AI UGC
automations, but there `requestId` is required. To replay the stage graph directly, call
...
```bash
curl -X POST https://<your-host>/api/automations/run \
...
`automationId` and `force: true` are both required; the route answers 400 without them. Optional
...
Open `/app?view=automations` and select the slideshow automation, or deep link straight to it with
`/app?view=automations&automation=automation_123`.
...
Click **Generate**. The drawer posts to `/api/automations/run` and the finished draft appears under
...
<Callout type="warn">
Similarity rewriting happens at most once. The retry result becomes authoritative; the pipeline
does not calculate or expose a second similarity score after rewriting.
</Callout>
