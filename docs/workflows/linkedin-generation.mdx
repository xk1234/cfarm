...
To run a single stage, call `lumenclip_pipeline_stage_run` with a stage ID such as
`linkedin-generation.validate-input` and an explicit `input` object. Linear `startAt` and
`stopAfter` windows are rejected for the DAG because they cannot preserve input-branch dependencies.
