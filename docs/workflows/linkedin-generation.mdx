...
**Availability:** `POST /api/linkedin-templates/generate` executes this pipeline without
...
queues `lumenclip - LinkedIn generation` in Windmill. Its audience/topic, voice/proof/model,
brief, and batch controls run real normalization and validation handlers before their first common
consumer. Downstream stages remain sequential where each genuinely depends on the previous result.
...
To run a single stage, use Windmill MCP's `runScriptByPath` on
`f/lumenclip/workflow_stage_runtime` with a stage ID such as
`linkedin-generation.validate-input` and an explicit `stage_input` object.
Linear `startAt` and `stopAfter` windows are rejected for the DAG because they
cannot preserve input-branch dependencies.
...
```bash
curl -X POST https://<your-host>/api/linkedin-templates/generate \
...
`POST /api/linkedin-templates/generate`, so use the HTTP or MCP path above.
...
<Callout type="warn">
  Repair is capped at three total generation attempts. When the third attempt
  still violates the deterministic gate, the post is returned with `needsReview`
  set and the last violations attached.
</Callout>
