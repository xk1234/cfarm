...
| -------------------- | ------------------------------------- | ----------------- |
| Resolve components   | Template plus six overrides           | `components`      |
| Analyze product      | Product component                     | `analysis`        |
| Generate script plan | `analysis`                            | `script`          |
| Resolve actor        | `analysis`, `script`                  | `actor`           |
| Animate actor        | `actor`                               | `motion`          |
| Synthesize voice     | `script`                              | `voice`           |
| Generate B-roll      | `script`                              | `broll`           |
| Lip-sync performance | `motion`, `voice`                     | `lipsync`         |
| Composite output     | `script`, `voice`, `lipsync`, `broll` | `composite`       |
| Store final draft    | `script`, `composite`                 | `store`           |

The Windmill graph first splits template, product, script, actor, voice, B-roll, and render inputs
into named input artifacts. Its `branchall` media section then runs actor/motion, voice, and B-roll
in parallel. Only named checkpoint artifacts cross the lip-sync, composite, and storage joins. Every
component job is polled to a terminal checkpoint inside its node, so the flow no longer stops after
merely enqueuing its first stage.
