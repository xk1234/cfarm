...
The Windmill graph uses a `branchall` media section for actor/motion, voice, and B-roll. It executes
those branches sequentially for now because checkpoint persistence is shared; the graph still makes
their independent contracts visible and keeps the join dependencies explicit. Every component job
is polled to a terminal checkpoint inside its node, so the flow no longer stops after merely
enqueuing its first stage.
