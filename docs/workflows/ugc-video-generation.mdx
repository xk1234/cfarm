...
**Processing:** select a portrait from the configured photo collection when supplied; otherwise generate a
portrait from actor settings and product analysis, download it, and persist `actor.png`.

**Model/provider:** generated branch uses `fal-ai/flux-2-pro` via fal.ai; collection branch
uses no model.
