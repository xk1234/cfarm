...
<Callout type="warn">
  ...
  worker and the MCP process, or generation refuses to start and the
  `/api/ugc-runs/*` routes answer 404. The template must also be `live` and pass
  its UGC configuration checks.
</Callout>
...
three attempts. `lumenclip_template_run` accepts the same UGC template and forwards to the same
...
Open the automation at `/app?view=templates&automation=automation_ugc_123` and configure it in the
