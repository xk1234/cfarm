...
`workflowId: "x-threads-generation"`. To test one stage, use Windmill MCP's
`runScriptByPath` on `f/lumenclip/workflow_stage_runtime` with a `stage_id` such
as `x-threads-generation.validate-input`.
