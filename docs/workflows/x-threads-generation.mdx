...
<Callout type="warn">
Every trigger below requires a persisted niche brief on the automation. Without one, generation
stops with `Generate the niche strategy before creating a draft`. Derive it first with
`POST /api/x-automations/<id>/derive-brief`, or with **Generate strategy** in the studio.
</Callout>
...
`automationId` and `requestId` are both required; `requestId` is the idempotency key, so reusing it
returns the same draft instead of generating a second one. Do not pass `hook`: it is accepted only
for slideshow automations and otherwise fails with `Explicit hooks are supported only for slideshow
runs`.
...
```bash
curl -X POST https://<your-host>/api/x-automations/generate \
...
Only `automationId` is required. `topic` overrides topic selection and `sourceCandidate` supplies a
trend record to react to. A successful call answers 201 with `{ "run": ... }`. The same path also
serves `GET` for stored runs and `DELETE` to clear an automation's runs and usage memory.
...
Open `/app/x-automations`, or open the automation from the workspace with
`/app?view=automations&automation=x-auto-01`, which embeds the same studio.

</Step>

<Step>

Set the niche, then click **Generate strategy** if the automation has no brief yet.
...
`/api/x-automations/generate`, and shows the new run in the overview tab.
