...
PostFast backend/client/data/API work is implemented and the todo was deleted per the repo's closed-task convention.
...
- Keep `POSTFAST_API_KEY` server-only. Do not expose it through `NEXT_PUBLIC_*` env vars or client components.
- PostFast posting should default to `draft` unless the user explicitly chooses `schedule` or `now`.
...
Agent 3 PostFast backend work does not need these files.
...
PostFast publish controls for generated Greenscreen or UGC exports should coordinate with the generated export contract.
...
- `lib/postfast-client.ts`
  - New server-side PostFast API wrapper.
  - Read `POSTFAST_API_KEY` and `POSTFAST_API_URL`.
...
- `lib/postfast-provider-settings.ts`
...
- `lib/postfast-posts.ts`
  - New local persistence helper for PostFast post mapping records.
  - Store local source references, PostFast post IDs, integration IDs, status, schedule date, release URL, media references, analytics cache, sync timestamps, and errors.

- `lib/postfast-client.test.ts`
  - New tests for client URL building, auth headers, request bodies, and error normalization.

- `lib/postfast-posts.test.ts`
  - New tests for local PostFast mapping persistence, status updates, and analytics cache updates.

- `data/postfast-posts.json`
  - New local JSON DB for PostFast post mappings and analytics cache.
...
## Safe PostFast API Routes

These are new route folders and should not collide with other agents:

- `app/api/postfast/integrations/route.ts`
- `app/api/postfast/connect-url/route.ts`
- `app/api/postfast/find-slot/route.ts`
- `app/api/postfast/upload/route.ts`
- `app/api/postfast/posts/route.ts`
- `app/api/postfast/analytics/platform/route.ts`
- `app/api/postfast/analytics/post/route.ts`
...
- `components/realfarm/calendar-analytics.tsx`
  - PostFast-backed schedule and analytics data.
...
- `components/realfarm/postfast-channel-selector.tsx`
  - New reusable component for connected integration/channel selection.

- `components/realfarm/postfast-status-badge.tsx`
  - New reusable status display for draft/scheduled/published/failed.

- `components/realfarm/postfast-analytics-model.ts`
  - New helper to normalize PostFast analytics into UI-safe shapes.

- `components/realfarm/postfast-analytics-model.test.ts`
  - New tests for analytics normalization.

- `docs/tabs/schedule.md`
  - Update schedule behavior from mock rows to PostFast-backed posts.

- `docs/tabs/analytics.md`
  - Update analytics behavior from mock data to PostFast-backed platform/post analytics.
...
1. Build and test the server-only PostFast client and provider settings helpers.
2. Build and test local PostFast post mapping persistence.
3. Add `/api/postfast/*` route handlers.
4. Add schedule/analytics PostFast sync UI only if it can stay inside `calendar-analytics.tsx`.
