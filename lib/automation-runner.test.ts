...
describe("runDueAutomations", () => {
  ...
  it("runs the real slideshow pipeline with sanitized schemas, legible glyphs, and idempotent persistence", async () => {
    ...
    // Ink alone cannot catch the production failure: this host has system
    // fonts, so deleting the bundled TTF still renders legible glyphs here
    // while the Alpine function container renders tofu. Assert the pipeline
    // actually resolved and configured the BUNDLED font, which is the property
    // that broke.
