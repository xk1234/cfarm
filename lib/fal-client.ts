...
export async function falSubmitAndWait<T>(input: {
  ...
  const requestId =
    input.requestId || (await falCreateTask({ ...input, fetchImpl }))
  if (!requestId)
...
export async function falGetTaskStatus(input: {
  ...
  const endpoint = input.endpoint.replace(/^\/+|\/+$/g, "")
  return falJson<{ status?: string; error?: string }>(
    input.fetchImpl ?? fetch,
...
export async function falGetTaskResult<T = Record<string, unknown>>(input: {
  endpoint: string
  requestId: string
  apiKey: string
  fetchImpl?: FetchLike
...
export type FalImageInput = Parameters<typeof falSubmitAndWait>[0] & {
  endpoint: string
}
...
export async function generateFalImage(input: FalImageInput) {
  return normalizeFalAsset(
    await falSubmitAndWait<Record<string, unknown>>(input),
    "image"
  )
}
...
export function normalizeFalAsset(
  payload: Record<string, unknown>,
  kind: "image" | "video"
): FalAsset {
  const candidate =
    kind === "image" && Array.isArray(payload.images)
      ? payload.images[0]
