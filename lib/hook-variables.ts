...
] as const satisfies readonly RuntimeHookVariableDefinition[]
...
export function hookVariableNameFromLabel(value: unknown) {
  return String(value ?? "")
    .trim()
    .replace(/^\[\[|\]\]$/g, "")
    .toLowerCase()
...
export function wordCollectionVariableName(
  collection: Pick<WordCollectionRecord, "id" | "name">
) {
  ...
  return (
    hookVariableNameFromLabel(collection.name) ||
...
export function migrateLegacyHookVariableReferences(input: {
  ...
  const text = input.text.replace(
    ...
    (token, bracketSlot, braceSlot) => {
      ...
      if (!collection || !legacyWordCollectionId(collection.id)) return token
      ...
      for (const key of Object.keys(hookSlots)) {
        if (key.toLowerCase() === slot.toLowerCase()) delete hookSlots[key]
      }
...
function legacyWordCollectionId(value: string) {
...
function isoDate(now: Date, timeZone: string | undefined) {
