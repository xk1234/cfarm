...
export type UGCProductAnalysis = {
  product: string
  audience?: string[]
  pains?: string[]
...
export type UGCScriptPlan = {
  hook: string
  segments: UGCScriptSegment[]
  caption: string
  hashtags: string[]
  hookOverlay?: string
  durationSeconds?: number
...
export async function resolvePublicProductUrl(value: string) {
...
export async function fetchProductPageResponse(input: {
  url: string
  timeoutMs?: number
  maxBytes?: number
  fetchImpl?: FetchLike
  ...
  try {
    ...
    return {
      redirectUrl: "",
      page: { url: current.toString(), title, description, text },
    }
  } finally {
    clearTimeout(timer)
  }
...
export async function analyzeUgcProduct(input: {
  apiKey: string
  productUrl?: string
  productBrief?: string
  fetchImpl?: FetchLike
}): Promise<UGCProductAnalysis> {
  const brief = input.productBrief?.trim() ?? ""
  const page = input.productUrl
...
export async function analyzeUgcProductFacts(input: {
  apiKey: string
  productBrief?: string
  page?: { url: string; title: string; description: string; text: string }
  fetchImpl?: FetchLike
}): Promise<UGCProductAnalysis> {
  const brief = input.productBrief?.trim() ?? ""
...
export function validateUgcScriptPlan(
  value: unknown,
  ...
  const segments = Array.isArray(record.segments)
    ? record.segments.map((item) => item as UGCScriptSegment)
    : []
  for (const phase of ["hook", "problem", "solution", "cta"] as const)
    if (
      !segments.some(
  ...
  return {
    ...
    hashtags: Array.isArray(record.hashtags)
      ? record.hashtags.map(String).slice(0, 12)
      : [],
    hookOverlay:
      typeof record.hookOverlay === "string" ? record.hookOverlay : undefined,
    durationSeconds: duration,
  }
}

function normalizePublicProductUrl(value: string) {
  let url: URL
  try {
    url = new URL(value)
  } catch {
