...
export class UgcConfigurationError extends Error {
  ...
  constructor(message: string, options: { telegramNotified?: boolean } = {}) {
    super(message)
...
export const ugcRunId = (automationId: string, scheduledFor: string) =>
  `ugcrun${hash(`${automationId}:${scheduledFor}`, 29)}`
export const ugcExportId = (automationId: string, scheduledFor: string) =>
  `ugc-${hash(`${automationId}:${scheduledFor}`, 32)}`

export type UgcStageName =
  | "analysis"
  | "script"
  | "actor"
  | "voice"
  ...
  | "store"
  | "publish"
export const ugcStageOrder: readonly UgcStageName[] = [
  "analysis",
  "script",
  "actor",
  "voice",
...
export async function runUgcAutomation(input: {
  ...
  saveCheckpoint?: (
    stage: UgcStageName,
    checkpoint: UgcCheckpoint,
    checkpoints: UgcCheckpoints
  ) => Promise<void>
  stopAfter?: UgcStageName
  onlyStages?: readonly UgcStageName[]
}) {
  const runId = ugcRunId(input.automationId, input.scheduledFor),
    exportId = ugcExportId(input.automationId, input.scheduledFor)
  if (
    input.automation.status !== "live" ||
...
export async function checkpointIsDurable(
  checkpoint: UgcCheckpoint,
  assetExists?: (path: string) => Promise<boolean>
) {
  const paths = [
  ...
  return (await Promise.all(paths.map((path) => assetExists(path)))).every(
    Boolean
  )
}
