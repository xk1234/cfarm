...
export const linkedInArchetypes: LinkedInArchetype[] = [
  ...
  },
  {
    ...
    personaSafe: true,
    structure:
      "framework promise → numbered steps with context → objective → question",
    template:
  ...
  },
  {
    ...
    template:
      "[Topic] needs less of this: - [thing] x3. And more of: - [thing] x3.",
    slots: [
  ...
  },
  {
    ...
    personaSafe: true,
    structure:
      "bad [topic]: 3 things → good [topic]: 3 things → anything you'd add?",
    template:
      "Bad [topic]: - thing x3. Good [topic]: - thing x3. Anything you'd add?",
    slots: [
  ...
  },
  {
    ...
    personaSafe: true,
    structure:
      "N things that destroy [thing ICP cares about] → avoid at all costs → open question",
    template:
      "[X] things that destroy a [topic thing]: 1..N (inverted best practices). Avoid at all costs. Anything you'd add?",
    slots: [
  ...
  },
  {
    ...
    personaSafe: true,
    structure:
      "topic 101 → nobody cares about X → they care about Y → short expansion",
    template:
      "[Topic] 101: Nobody cares about [common obsession]. They care about [what actually matters]. [2-4 lines of expansion]",
    slots: [
  ...
  },
  {
    ...
    template:
      "In 5 words or less, what advice would you give [type of person]?",
    slots: [
  ...
  },
  {
    ...
    needsProof: true,
    structure:
      "timeframe + risk taken → context → obstacle → turning point → now → lesson",
    template:
      "[Timeframe] ago I [risk]. [Context] [Obstacle] [Reality] [Turning point] Now: [results]. [Lesson]",
    slots: [
      slot(
        "hook",
  ...
  },
  {
    ...
    needsProof: true,
    structure:
      "my old [topic]: 3 things → my new [topic]: 3 things → emotional contrast → small change big impact",
    template:
      "My old [topic]: x3. My new [topic]: x3. [Old emotion/result] Now [new result]. Small change, big impact.",
    slots: [
...
export const linkedInHookStyles: LinkedInHookStyle[] = [
  ...
  },
  {
    ...
    formula:
      "[observable failure] -> [specific outcome without the felt obstacle]",
    examples: [
  ...
  },
  {
    ...
    label: "Harsh truth",
    formula:
      "[uncomfortable diagnosis or scapegoat reversal; literal label optional]",
  ...
  },
  {
    ...
    formula:
      "[recognizable scene, quote, or contradiction exposing the feared problem]",
    examples: [
...
const mechanic = (
  ...
  instruction: string,
  example: string
): Mechanic => ({ id, instruction, example })
...
const cellHookExamples: Record<string, Record<string, string>> = {
  ai: {
    how_to_without:
      "Third AI draft open, cursor back at line one? Fix the inputs before rewriting again.",
    struggles_advice:
      "You delete the AI opening before reading line two. Your voice setup left it no better option.",
    harsh_truth:
      "You paste in a blog. Five posts come back wearing the same opening. The model is not the problem.",
    process_breakdown:
      "Three AI tabs are open. The post is still a blank document. Shrink the workflow first.",
  },
  web: {
    how_to_without:
      "The site is live. The owner still checks a silent inbox. Fix the path before redesigning.",
    struggles_advice:
      "A homeowner scrolls once, then calls the competitor. The trust proof arrived too late.",
    harsh_truth:
      "Your booking form asks for a budget before it earns a phone number. New colors will not fix that.",
    process_breakdown:
      "The DIY site looks finished. On mobile, the quote button takes three scrolls to reach.",
  },
  career: {
    how_to_without:
      "Your RFC gets approvals, then the same decision happens in Slack without you.",
    struggles_advice:
      "Review week arrives and your brag doc reads like Jira history. That is not a promotion case.",
    harsh_truth:
      "Five tickets closed. Your manager still calls you reliable, never senior. That is the real gap.",
    process_breakdown:
      "Planning starts tomorrow. Your architecture proposal still lives in six Slack threads.",
  },
}
...
const cellOutcomeAnchors: Record<string, Record<string, string>> = {
  ai: {
    how_to_without:
      "a usable AI draft that preserves the solo creator's recognizable voice without another rewrite loop",
    struggles_advice:
      "a repeatable voice input that stops generic drafts before the solo creator has to rescue them",
    harsh_truth:
      "standalone repurposed posts built around distinct claims rather than summaries of one source",
    process_breakdown:
      "a voice-consistent batch moved from raw material into the scheduler in one focused session",
  },
  web: {
    how_to_without:
      "a mobile visitor reaching a clear call or quote action without a full redesign",
    struggles_advice:
      "a homeowner seeing verifiable local trust before deciding to call a competitor",
    harsh_truth:
      "a booking form that asks only for what is needed to begin the service conversation",
    process_breakdown:
      "a homeowner moving from the landing page to a working mobile booking path before visual polish",
  },
  career: {
    how_to_without:
      "an architecture decision that records the engineer's judgment and earns visible ownership",
    struggles_advice:
      "promotion evidence that turns vague manager feedback into named senior-level behaviors",
    harsh_truth:
      "a self-review bullet that connects engineering judgment to a business or cross-team change",
    process_breakdown:
      "one promotion-ready design artifact that demonstrates cross-team judgment before planning begins",
  },
}
...
function nicheKeyForPillar(pillar: string): "ai" | "web" | "career" | null {
  ...
  if (/local|conversion|booking|social proof|web design|DIY/i.test(pillar))
    return "web"
  if (
    /technical|career|senior|promotion|burnout|cross-functional|stakeholder/i.test(
      pillar
    )
  )
    return "career"
...
export function selectMechanics(plan: LinkedInPostPlan) {
  ...
  const nicheOffset = nicheKey
    ? { ai: 0, web: 1, career: 2 }[nicheKey]
    : undefined
  const pick = (pool: Mechanic[], kind: string) => {
    if (!pool.length) return null
    const familyIndex = stableIndex(
      `${plan.archetype.id}|${plan.hookStyle.id}|${kind}`,
      pool.length
    )
...
export function voicePresetById(id: string) {
  return (
    linkedInVoicePresets.find((item) => item.id === id) ??
    linkedInVoicePresets[0]
  )
}
...
export function buildLinkedInSystemPromptVariables(input: {
  ...
  const proofText = input.proof?.length
