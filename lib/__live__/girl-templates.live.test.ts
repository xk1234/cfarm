...
const liveTemplates = process.env.RUN_LIVE
  ? await listAutomationTemplateRecords()
  : []
const liveExampleRuns = process.env.RUN_LIVE
  ? await listAutomationTemplateExampleRuns()
  : []

describe.skipIf(!process.env.RUN_LIVE)(
  "LIVE Grid/Horizontal Girl vs example outputs",
  () => {
    for (const template of liveTemplates.filter((item) =>
      /girl/i.test(item.name)
    )) {
      it(`generates and compares: ${template.name}`, async () => {
        ...
        const examples = liveExampleRuns.filter((run) =>
          String(run.templateId).includes(idFrag)
        )
        ...
        const bodyKeys = placeholders
          ...
          .map((p: any) => p.id)
        const titleWCs = titleKeys.map((k: string) =>
          wc(res.result.text[k] ?? "")
        )
        const bodyWCs = bodyKeys.map((k: string) =>
          wc(res.result.text[k] ?? "")
        )
        ...
        console.log(
          `LAYOUT   generated ${genSlides} slides [${sections}]  vs  example ${exampleSlideCount} slides`
        )
        console.log(
          `         ${genSlides === exampleSlideCount ? "MATCH" : "MISMATCH — generated is missing the trailing CTA slide (cta.enabled=false but image_collection_ids.cta_slide.check=true)"}`
        )
        ...
        console.log(
          `HOOK     example: "${exampleHook}" (${wc(exampleHook)} words)`
        )
        console.log(`WORDS    titles (spec 1-2): [${titleWCs.join(", ")}]`)
        console.log(`         bodies (spec 6-10): [${bodyWCs.join(", ")}]`)
        console.log(
          `STYLE    all-lowercase=${allLower}  first-person=${firstPerson}  discourse-markers=${hasDiscourse}`
        )
        ...
        for (const [slide, parts] of Object.entries(bySlide)) {
          console.log(
            `   ${slide}: [${parts.map((x) => JSON.stringify(x)).join("  |  ")}]`
          )
        }
