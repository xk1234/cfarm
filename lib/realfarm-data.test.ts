...
describe("loadRealFarmData", () => {
  it("loads the JSON DB shape needed by the RealFarm workspace", () => {
    ...
    expect(data.editor.slides).toEqual([])
    ...
    expect(data.assets.ugcAvatarVideos[0].url).toMatch(/^\/api\/local-assets\/ugc_avatar_videos\/.+\.mp4$/)
    expect(getOrderedUgcAvatarVideos(data)[0].url).toBe("/api/local-assets/ugc_avatar_videos/323e6dbd-d758-47cb-90a0-a0882c24edf8.mp4")
