...
export { CollectionDetailView } from "@/features/collections/ui/collection-detail-view"
...
} from "@/features/collections/ui/collection-loading-states"
import {
  collectionToStored,
...
const CollectionDataTable = dynamic<{
  rows: CollectionTableRow[]
  columns: ColDef<CollectionTableRow>[]
  getRowId: (row: CollectionTableRow) => string
  onRowClick: (row: CollectionTableRow) => void
  emptyMessage: string
}>(
  () =>
    import("@/components/ui/ag-data-table").then(
      (module) => module.AgDataTable
    ),
  { loading: () => <CollectionTableSkeleton /> }
)
