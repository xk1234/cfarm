...
import { calendarTimingEntries } from "@/features/calendar/domain/calendar"
import {
  calendarAccountOptions,
  calendarAutomationOptions,
  calendarHumanize,
  calendarItemMatchesViewFilters,
  calendarLifecycleOptions,
  calendarPlatformOptions,
  calendarProviderLabel,
  calendarSourceTypeOptions,
  countCalendarViewFilters,
  defaultCalendarViewFilters,
  hasCalendarViewFilters,
  isFailedCalendarStatus,
...
export function ContentCalendar() {
  ...
  const [filters, setFilters] = useState<CalendarViewFilters>(
    defaultCalendarViewFilters
  )
  ...
  useEffect(() => {
    try {
      ...
      if (stored) setFilters(normalizeStoredCalendarFilters(JSON.parse(stored)))
    } catch {
  ...
  const accounts = useMemo(() => calendarAccountOptions(items), [items])
  const platforms = useMemo(() => calendarPlatformOptions(items), [items])
  const automations = useMemo(() => calendarAutomationOptions(items), [items])
  const sourceTypes = useMemo(() => calendarSourceTypeOptions(items), [items])
  const activeFilters = useMemo(
    () =>
      reconcileCalendarViewFilters(filters, {
        accounts,
        automations,
        platforms,
        sourceTypes,
  ...
  const visibleItems = useMemo(
    () =>
      items.filter((item) =>
        calendarItemMatchesViewFilters(item, activeFilters)
      ),
    [activeFilters, items]
  )
  const visibleSummary = useMemo(
    () => ({
      needsAction: visibleItems.filter((item) => item.status === "needs_action")
        .length,
      failed: visibleItems.filter((item) => isFailedCalendarStatus(item.status))
        .length,
  ...
  const activeFilterCount = countCalendarViewFilters(activeFilters)
      ...
      <>
        ...
        <FilterControlSlot label="Lifecycle" mobile={mobile}>
          <MultiSelectFilter
            ...
            options={calendarLifecycleOptions}
    ...
    <div className="mx-auto max-w-[1380px] pb-12">
      ...
      <section className="mb-4 hidden rounded-[12px] border border-app-panel-border bg-app-surface p-3 md:block">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {hasCalendarViewFilters(activeFilters) ? (
            <button
              type="button"
              onClick={() => setFilters(defaultCalendarViewFilters)}
...
function renderCalendarEvent(info: EventContentArg) {
  const item = info.event.extendedProps.item as CalendarItem
  const provider = primaryCalendarProvider(item)
  return (
    <div
        ...
        >
          <article className="p-5 sm:p-6">
            ...
            <dl className="mt-5 grid grid-cols-[92px_1fr] gap-y-3 border-y border-[#eceae3] py-4 text-[12px]">
              ...
              <DetailRow
                label="Source"
                value={calendarHumanize(item.sourceType)}
              />
                      ...
                      <div className="min-w-0">
                        <div className="truncate text-[11px] font-semibold text-app-text">
                          {target.integrationName ||
                            calendarProviderLabel(target.provider)}
                        </div>
...
function StatusBadge({ status }: { status: CalendarLifecycleStatus }) {
  return (
    ...
    >
      <span
        className={cn("size-1.5 rounded-full", lifecycleDotClass(status))}
      />
      {statusLabel(status)}
    </span>
  )
...
function lifecycleBadgeClass(status: CalendarLifecycleStatus) {
  if (isFailedCalendarStatus(status)) return "bg-[#fde9e5] text-[#8e342c]"
...
function lifecycleDotClass(status: CalendarLifecycleStatus) {
  if (isFailedCalendarStatus(status)) return "bg-[#c94f42]"
...
function statusLabel(status: CalendarLifecycleStatus) {
  return calendarHumanize(status)
}
...
function timestampLabel(value: string) {
  ...
  return labels[value] || calendarHumanize(value)
}
