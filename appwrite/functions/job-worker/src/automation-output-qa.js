// Generated from lib/automation-output-qa.ts. Do not edit by hand.
import { automationFormatSection, } from "./slideshow-plan-core.js";
import { isRuntimeHookVariable } from "./hook-variables.js";
import { wordRangeViolation } from "./temp-slide-testing-shared.js";
...
function countMismatchFindings(run, bodySlideCount) {
    ...
    const explicitCounts = substitutions.flatMap(([name, rawValue]) => {
        if (!countTokenPattern.test(name))
            return [];
...
function unresolvedTokenFindings(run) {
    ...
    run.plan.slides.forEach((slide, slideIndex) => {
        if (slide.textItems?.length) {
            slide.textItems.forEach((item) => values.push({ text: item.text, slideIndex, textItemId: item.id }));
        }
        else {
    ...
    return values.flatMap((value) => {
        ...
        return tokens.map((token) => ({
            ...
            severity: "error",
            slideIndex: value.slideIndex === undefined ? undefined : value.slideIndex + 1,
...
function duplicateVariableDrawFindings(run) {
    ...
    for (const [name, rawValue] of Object.entries(run.plan.hookSubstitutions ?? {})) {
    ...
    return [...byValue.entries()].flatMap(([value, names]) => names.length < 2
        ? []
        : [
            {
...
function slideTextFindings(slides, schema) {
    return slides.flatMap((slide, slideIndex) => {
        ...
        const section = schema
            ? automationFormatSection(schema, slide.role === "hook"
                ? "hook"
                : slide.role === "cta"
                    ? "cta"
                    : "content")
            : undefined;
        if (section?.noText || slide.displayText === false)
            return [];
        ...
        const configuredById = new Map((section?.textItems ?? []).map((item) => [item.id, item]));
        return renderedItems.flatMap((item, itemIndex) => {
            const text = item.text.trim();
            const configured = configuredById.get(item.id) ?? section?.textItems[itemIndex];
            if (!text) {
                return [
                    {
                        code: "EMPTY_SLIDE_TEXT",
...
function wordLengthFindings(text, configured, slideIndex, textItemId) {
    const words = text.split(/\s+/).filter(Boolean).length;
    const direction = wordRangeViolation(words, configured.wordLengthMin, configured.wordLengthMax);
    if (!direction)
        return [];
    const limit = direction === "below" ? configured.wordLengthMin : configured.wordLengthMax;
    return [
        {
