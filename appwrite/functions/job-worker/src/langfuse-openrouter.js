// Generated from lib/langfuse-openrouter.ts. Do not edit by hand.
import { propagateAttributes, startActiveObservation, } from "@langfuse/tracing";
import { LANGFUSE_APP_NAME } from "./langfuse-config.js";
export async function tracedOpenRouterFetch(name, url, init, context) {
    ...
    }, () => startActiveObservation(name, async (generation) => {
        generation.update({
...
export function openRouterOperationName(body, fallback = "generate-content") {
...
function modelParameters(body) {
    ...
    return Object.fromEntries(Object.entries(values).filter((entry) => typeof entry[1] === "number"));
}
function usageDetails(usage) {
    ...
    if (promptTokens !== undefined &&
        ...
        return {
            ...
            ...numericDetails("completion_tokens_details", usage.completion_tokens_details),
        };
    }
    const values = {
    ...
    const details = Object.fromEntries(Object.entries(values).filter((entry) => typeof entry[1] === "number"));
...
function numericDetails(key, value) {
    ...
    if (!record)
        return {};
    const details = Object.fromEntries(Object.entries(record).filter((entry) => typeof entry[1] === "number" && Number.isFinite(entry[1])));
...
function providerError(value, status) {
    ...
    return redactSensitiveText(stringValue(error?.message)?.slice(0, 500) ?? `OpenRouter HTTP ${status}`);
}
