// Generated from lib/slideshow-font-family.ts. Do not edit by hand.
 ...
 */
export const SLIDESHOW_FONT_FACES = [
    {
        family: "Inter",
        label: "Inter",
...
const AVAILABLE_FONT_FAMILIES = new Set(SLIDESHOW_FONT_FACES.map(({ family }) => family));
const FONT_WEIGHTS = new Map(SLIDESHOW_FONT_FACES.map(({ family, weight }) => [family, weight]));
 ...
 */
export function resolveSlideshowFont(requested) {
    if (!requested)
        return BUNDLED_FONT_FAMILY;
    if (AVAILABLE_FONT_FAMILIES.has(requested))
        return requested;
    const replacement = FONT_REPLACEMENTS[requested];
    if (replacement)
        return replacement;
    if (CSS_GENERIC_FAMILIES.has(requested.toLowerCase()))
        return requested;
    if (requested === "Arial")
        return requested;
    return BUNDLED_FONT_FAMILY;
}
export function resolveSlideshowFontWeight(requested, requestedWeight) {
    if (Number.isFinite(requestedWeight)) {
        return Math.max(100, Math.min(900, Math.round(requestedWeight / 100) * 100));
    }
