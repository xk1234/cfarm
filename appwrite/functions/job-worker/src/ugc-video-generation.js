...
export function validateUgcScriptPlan(value, targetDurationSeconds = 60) {
    ...
    const record = value;
    const segments = Array.isArray(record.segments) ? record.segments.map((item) => item) : [];
...
const decodeEntities = (value) => value.replace(/&nbsp;/gi, " ").replace(/&amp;/gi, "&").replace(/&lt;/gi, "<").replace(/&gt;/gi, ">").replace(/&#39;/gi, "'").replace(/&quot;/gi, '"').trim();
