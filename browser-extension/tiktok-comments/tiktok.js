...
function hardStopIfBlocked() {
  ...
  if (/drag the slider to fit the puzzle|captcha|verify to continue/i.test(text)) {
    throw new Error("TikTok CAPTCHA detected. Open TikTok and complete it yourself; the companion has stopped.")
  }
  if (/log in to comment/i.test(text)) {
    throw new Error("TikTok login required. Sign in yourself; the companion has stopped.")
  }
}

async function openComments() {
  document.querySelector('[data-e2e="comment-icon"]')?.click()
  await waitFor(() => document.querySelector('div[class*="DivCommentObjectWrapper"]') || document.querySelector('[data-e2e="comment-input"]'))
}
...
async function collectComments() {
  ...
  const nestedReplyCount = comments.reduce((sum, item) => sum + item.replyCount, 0)
  const headerText = document.querySelector('[data-e2e="comment-count"]')?.textContent || ""
  return {
    comments,
    complete: {
...
function extractComment(wrapper) {
  ...
  const replyText = [...wrapper.querySelectorAll("*")].map((node) => node.textContent?.trim()).find((text) => /^View \d+ replies$/i.test(text || "")) || ""
  const likeLabel = [...wrapper.querySelectorAll('[aria-label^="Like video"]')].map((node) => node.getAttribute("aria-label")).find(Boolean) || ""
  const dateText = [...wrapper.querySelectorAll("span")].map((node) => node.textContent?.trim()).find((text) => /^\d{1,2}-\d{1,2}$/.test(text || ""))
  const id = wrapper.getAttribute("data-comment-id") || wrapper.querySelector('a[href*="/comment/"]')?.getAttribute("href")?.match(/comment\/(\d+)/)?.[1] || hash(`${href}:${textNode.textContent}:${dateText || ""}`)
  return {
    tiktokCommentId: id,
    displayName: wrapper.querySelector('[data-e2e="comment-username-1"]')?.textContent?.trim() || "",
...
async function sendApproved(send) {
  ...
  if (!comment || !send?.text) throw new Error("Approved send payload is incomplete")
  const wrapper = [...document.querySelectorAll('div[class*="DivCommentObjectWrapper"]')].find((node) => {
  ...
  const input = await waitFor(() => document.querySelector('[data-e2e="comment-input"]'))
  input.focus()
  document.execCommand("insertText", false, send.text)
  input.dispatchEvent(new InputEvent("input", { bubbles: true, inputType: "insertText", data: send.text }))
...
function parseCount(value) {
  ...
  return Math.round(number * ({ K: 1e3, M: 1e6, B: 1e9 }[match[2]?.toUpperCase()] || 1))
}
function waitFor(read, timeout = 10000) {
  return new Promise((resolve, reject) => {
    ...
    const tick = () => {
      ...
      else if (Date.now() - start > timeout) reject(new Error("TikTok comment panel did not become ready"))
