/**
 * Explicit opt-in command for copying user-owned template definitions from
 ...
 *
 * Owners are mapped by email. Existing local template ids are preserved so
...
const sourceTableId = "automations"
const targetTableId = "templates"
...
const [cloudUsers, localUsers, sourceRows, targetRows] = await Promise.all([
  ...
  listAllRows(cloud.tables, sourceDatabaseId, sourceTableId),
  listAllRows(local.tables, targetDatabaseId, targetTableId),
])
...
for (const [index, row] of sourceRows.entries()) {
  ...
  if (!cloudOwner || !localOwner) {
    throw new Error(
      `Cannot map template owner ${clean(row.owner_id) || "(missing)"} to a local account. Sign into the local app with the same email first.`
    )
  }

  const rid = clean(row.rid) || clean(record.id)
  if (!rid) {
    throw new Error(`Template row ${row.$id} has no domain id.`)
  }
...
console.log(
  `Template sync complete: ${imported} imported, ${skipped} existing local records preserved.`
)
...
function parseRecord(row) {
  ...
  throw new Error(`Template row ${row.$id} has invalid JSON data.`)
}
