...
export async function runUgcAutomationJob({
  ...
  const persist = (name, bytes, contentType) =>
    persistAsset(storage, `${prefix}/${name}`, bytes, contentType)
  const load = (path) => loadAsset(storage, path)
  const durableInput = async (path, contentType) =>
    dataUrl(await load(path), contentType)
  const usage = async (stage, detail = {}) =>
    recordUsage(tables, databaseId, ownerId, automationId, runId, stage, detail)
  try {
    return await runUgcAutomation({
      ...
      assetExists: async (storagePath) => {
        const fileId = crypto
          .createHash("sha256")
          .update(storagePath.replace(/^data\//, ""))
          .digest("hex")
          .slice(0, 36)
        try {
          await storage.getFile("ugc_videos", fileId)
          return true
        } catch {
          return false
        }
      },
      saveCheckpoint: async (stage, _checkpoint, all) =>
        upsertRun(tables, databaseId, ownerId, {
          ...(existingRun || {}),
          kind: "ugc",
          jobId: job?.$id || job?.id,
          id: runId,
      ...
      stages: {
        ...
        actor: async ({ checkpoints }) => {
          ...
          if (!sourceUrl || ugc.actorSource === "generate") {
            const asset = await api.image({
              ...
              input: {
                prompt: actorPrompt(ugc, checkpoints.analysis.analysis),
        ...
        voice: async ({ checkpoints }) => {
          const text = checkpoints.script.plan.segments
            .map((item) => item.spokenText)
            .join(" ")
          ...
          const asset = await api.lipsync({
            ...
            input: {
              video_url: await durableInput(
                checkpoints.motion.storagePath,
                "video/mp4"
              ),
              audio_url: await durableInput(
                checkpoints.voice.audioPath,
                "audio/mpeg"
              ),
            },
          })
        ...
        broll: async ({ checkpoints }) => {
          const candidates = checkpoints.script.plan.segments
            .filter((item) => item.brollPrompt)
            .slice(0, bounded(ugc.brollCount, 0, 6, 3))
          const assets = []
        ...
        composite: async ({ checkpoints }) => {
          ...
          const spec = buildUgcFfmpegCommand({
            durationSeconds: plan.durationSeconds || ugc.targetDurationSeconds,
            hook: plan.hookOverlay || plan.hook,
            captions:
              checkpoints.voice.words ||
              JSON.parse(
                new TextDecoder().decode(
                  await load(checkpoints.voice.timingsPath)
                )
              ),
            broll: checkpoints.broll.assets.map((item, index) => ({
          ...
          const rendered = await api.composite({
            ...
            spec,
            actor: await load(checkpoints.lipsync.storagePath),
            broll: await Promise.all(
              checkpoints.broll.assets.map((item) => load(item.storagePath))
            ),
            fetchImpl: api.fetch,
          })
        ...
        store: async ({ exportId, checkpoints }) => {
          const output = await upsertGeneratedOutput(
            ...
            ownerId,
            {
              ...
              runId,
              scheduledFor,
              plan: checkpoints.script.plan,
              checkpoints,
            }
          )
          await enqueueNotification(tables, databaseId, ownerId, {
            event: "generated",
            sourceId: exportId,
            runId,
          ...
          return {
            ...
            storagePaths: [
              checkpoints.composite.videoPath,
              checkpoints.composite.thumbnailPath,
            ],
          }
        },
        publish: async ({ exportId, checkpoints }) =>
          draftOnly
            ? { skipped: true, reason: "draft_only" }
            : publishOutput({
                tables,
                databaseId,
                ownerId,
...
async function downloadRemote(fetchImpl, url, allowedPrefixes) {
  ...
  if (!bytes.length || bytes.length > 250_000_000)
...
function fileId(path) {
  return crypto.createHash("sha256").update(path).digest("hex").slice(0, 36)
}
...
function classifyConfiguration(error, stage) {
  ...
  return new UgcConfigurationError(
    `UGC ${stage} configuration failed: ${message}`
  )
}
...
async function recordUsage(
  ...
  const now = nowIso(),
    id = `usage-${crypto.createHash("sha256").update(`${runId}:${stage}`).digest("hex").slice(0, 24)}`
  const record = {
    id,
  ...
  await tables.upsertRow(databaseId, USAGE, ownedRowId(USAGE, ownerId, id), {
    rid: id,
    created_raw: now,
    ord: -Date.now(),
    owner_id: ownerId,
...
async function upsertGeneratedOutput(tables, databaseId, ownerId, input) {
  ...
  await tables.upsertRow(databaseId, OUTPUTS, rowId, {
    ...
    name: record.title.slice(0, 2048),
    kind: "ugc_ad",
    subtype: "ugc_ad",
    status: "ready",
    ...
    hook: String(plan.hook || "").slice(0, 10000),
    caption: record.caption.slice(0, 100000),
    hashtags: JSON.stringify(record.hashtags),
    text: plan.segments
      .map((item) => item.spokenText)
      .join("\n")
      .slice(0, 100000),
    text_data: JSON.stringify(plan.segments),
  ...
  await syncOutputMedia(tables, databaseId, ownerId, rowId, [
    {
      role: "rendered_video",
      kind: "video",
...
async function syncOutputMedia(
  ...
  const response = await tables.listRows(databaseId, OUTPUT_MEDIA, [
    `equal(\"output_id\",[\"${outputRowId}\"])`,
    "limit(100)",
  ])
  for (const row of response.rows || [])
    await tables.deleteRow(databaseId, OUTPUT_MEDIA, row.$id)
  for (const [position, item] of media.entries()) {
    const relative = item.path.replace(/^data\//, ""),
      id = `m${crypto.createHash("sha256").update(`${outputRowId}:${item.role}`).digest("hex").slice(0, 35)}`
    await tables.createRow(databaseId, OUTPUT_MEDIA, id, {
...
async function publishOutput({
  ...
  const mode = ["auto", "review", "manual"].includes(schema.posting_mode)
    ? schema.posting_mode
    : "auto"
  const content = [
  ...
  if (mode === "auto") {
    ...
    await enqueueNotification(tables, databaseId, ownerId, {
      ...
      text:
        mode === "review"
          ? `UGC video ready for review\n${content}`
          : `UGC video ready to post\n${content}`,
    })
  }
  await updateOutputPublications(
    tables,
    databaseId,
    ownerId,
    exportId,
  ...
  return {
    outputId: exportId,
    status:
      mode === "auto" && records.length
        ? "posted"
        : mode === "review"
          ? "ready-for-review"
          : mode === "manual"
            ? "awaiting-manual-post"
            : "generated",
...
async function postFastRequest(path, body, fetchImpl) {
  let response
  try {
    response = await fetchImpl(`https://api.postfa.st${path}`, {
      method: "POST",
      headers: {
...
async function updateOutputPublications(
  ...
  const rowId = consolidatedRowId(
    ...
    row = await tables.getRow(databaseId, OUTPUTS, rowId),
    stored = safeJson(row.data) || {}
...
async function enqueueNotification(tables, databaseId, ownerId, input) {
  const settings = safeJson(
    (
      await tables.listRows(databaseId, ASSETS, [
        `equal(\"owner_id\",[\"${ownerId}\"])`,
  ...
  if (
    ...
    settings.events?.[input.event] !== true
  )
    return
  const dedupe = [
      "reminder",
      ...
      .filter(Boolean)
      .join(":"),
    id = `j${crypto.createHash("sha256").update(`${ownerId}:${dedupe}`).digest("hex").slice(0, 35)}`,
    now = nowIso()
  try {
    await tables.createRow(databaseId, JOBS, id, {
      ...
      max_attempts: 5,
      available_at:
        Date.parse(input.availableAt) > Date.now() ? input.availableAt : now,
      dedupe_key: dedupe,
      created_at: now,
      updated_at: now,
      owner_id: ownerId,
  ...
  return Object.fromEntries(
    ["actor", "voice", "motion", "lipsync", "broll", "composite"].map(
      (stage) => [
        stage,
        {
          provider: checkpoints[stage]?.provider,
          model: checkpoints[stage]?.model,
          requestId: checkpoints[stage]?.requestId,
        },
      ]
    )
  )
