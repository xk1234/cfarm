    ...
    <div className="min-h-full bg-app-surface px-9 py-8 pr-12">
      ...
      <Section title="Product input">
        <Field
          ...
          error={
            productMissing
              ? "Add a product URL or product brief before going live."
              : ""
          }
        >
          <input
        ...
        <div className="my-3 text-center text-xs font-bold tracking-wide text-app-text-faint uppercase">
          or
        </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="Target duration (seconds)">
            <input
              ...
              min={10}
              max={90}
              value={ugc.targetDurationSeconds}
              onChange={(event) =>
                update({ targetDurationSeconds: Number(event.target.value) })
              }
            />
          </Field>
          <Field label="B-roll count">
            <input
              ...
              min={0}
              max={6}
              value={ugc.brollCount}
              onChange={(event) =>
                update({ brollCount: Number(event.target.value) })
              }
            />
          </Field>
        </div>
      ...
      <Section title="On-screen text">
        <Toggle
          ...
          onToggle={() =>
            update({
              captions: { ...ugc.captions, enabled: !ugc.captions.enabled },
            })
          }
        />
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="Caption style">
            <input
              className={inputClass}
              value={ugc.captions.style}
              disabled={!ugc.captions.enabled}
              onChange={(event) =>
                update({
                  captions: { ...ugc.captions, style: event.target.value },
                })
              }
            />
          </Field>
          <Field label="Caption fallback">
            <SelectControl
              className="mt-2"
              value={ugc.captions.fallback}
              disabled={!ugc.captions.enabled}
              onChange={(event) =>
                update({
                  captions: {
                    ...
                    fallback: event.target
                      .value as AutomationUgcConfig["captions"]["fallback"],
                  },
                })
              }
            >
              <option value="drawtext">Draw text</option>
              <option value="png_frames">PNG frames</option>
            </SelectControl>
          </Field>
        </div>
        <Toggle
          label="Hook overlay"
          enabled={ugc.hookOverlay.enabled}
          onToggle={() =>
            update({
              hookOverlay: {
                ...ugc.hookOverlay,
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="Hook style">
            <input
              className={inputClass}
              value={ugc.hookOverlay.style}
              disabled={!ugc.hookOverlay.enabled}
              onChange={(event) =>
                update({
                  hookOverlay: {
                    ...ugc.hookOverlay,
          ...
          <Field label="Hook duration (milliseconds)">
            <input
              className={inputClass}
              type="number"
              min={500}
              max={10000}
              step={100}
              value={ugc.hookOverlay.durationMs}
              disabled={!ugc.hookOverlay.enabled}
              onChange={(event) =>
                update({
                  hookOverlay: {
      ...
      <Section title="Estimated provider cost">
        {estimate ? (
          ...
          <p className="text-sm font-medium text-app-muted-text">
            {estimateError || "Calculating estimate…"}
          </p>
        )}
      </Section>
      ...
      {validationErrors.length ? (
        <div
          role="alert"
          className="mt-6 rounded-lg border border-destructive/30 bg-destructive/10 p-3 text-sm font-semibold text-destructive"
        >
...
function Segmented<T extends string>({
  ...
  return (
    <fieldset>
      <legend className="text-sm font-semibold text-app-text">{label}</legend>
      <div className="app-segmented-control mt-2 inline-flex flex-wrap gap-1 rounded-lg bg-app-surface-subtle p-1">
        {options.map(([id, text]) => (
          <button
            key={id}
            type="button"
...
function CostEstimate({ estimate }: { estimate: UgcCostBreakdown }) {
  return (
    <div className="rounded-lg border border-app-panel-border p-4">
      <ul className="space-y-2">
        {estimate.items.map((item) => (
          <li
            key={`${item.stage}-${item.model}`}
            className="flex justify-between gap-4 text-sm"
          >
            <span
              className="min-w-0 truncate font-medium text-app-muted-text capitalize"
      ...
      <div className="mt-4 flex justify-between border-t border-app-panel-border pt-3 text-base font-bold text-app-text">
        <span>
          {estimate.tier === "premium" ? "Premium" : "Standard"} total
        </span>
        <span className="font-mono">{money(estimate.totalUsd)}</span>
      </div>
    </div>
  )
}
