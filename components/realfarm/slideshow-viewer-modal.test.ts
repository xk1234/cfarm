...
describe("SlideshowViewerModal", () => {
  it("shows the generated slide image without adding duplicate styled text", () => {
    ...
    expect(source).toContain("src={visibleSlide.imageUrl}")
  ...
  it("keeps every available slideshow action visible in the header", () => {
    ...
    expect(source).toContain("max-h-[clamp(300px,52vh,460px)]")
    expect(source).toContain("max-w-[min(72vw,760px)]")
    expect(source).toContain("object-contain")
    expect(source).not.toContain('className="aspect-[9/16]')
    expect(source).toContain('aria-label="Previous slide"')
    expect(source).toContain('aria-label="Next slide"')
